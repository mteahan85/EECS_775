...
public:
	...
	virtual void run() = 0;
	// if (vAR <= 0) then use all of viewport whenever reshape callback occurs.
	// else use the largest portion of the actual window that matches the given
	//      aspect ratio.
	// Any change will take effect with the next actual reshape callback.
	void setFixedViewportAspectRatio(double vAR);
...
private:
	...
	double fixedViewportAspectRatio;
};
